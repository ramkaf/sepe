...
import { DetailFieldIdDto, DetailField, UpdateDetailFieldDto, CreateDetailFieldDto } from '@sephrmicroservice-monorepo/common';
import { Repository } from 'typeorm';

@Injectable()
export class DetailFieldService {
    constructor(
        @InjectRepository(DetailField) private readonly detailFieldRepository: Repository<DetailField>
      ) {}
      ...
      async add(createDetailFieldDto: CreateDetailFieldDto): Promise<DetailField> {
        ...
        const detailField = this.detailFieldRepository.create({
            detailId , chartType , devideBy , oprType , fieldId , unit
        });
      ...
      async modify(updateDetailFieldDto: UpdateDetailFieldDto): Promise<DetailField> {
        const { dfId } =
          updateDetailFieldDto;
        const detailField = await this.detailFieldRepository.findOne({ where: { dfId } });
        if (!detailField) {
